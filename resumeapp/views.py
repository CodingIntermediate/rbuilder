...
def appliedusers(request):
    ...
    # Directly filter JobApplication instances based on regid through the job_id__regid field lookup
    applied_users = JobApplication.objects.filter(job_id__regid=regid)

    return render(request, 'Appliedusers.html', {'appliedusers': applied_users})
# not completed
# ================================================================================
...
# ------------------------------------------------------------------
# create a complaint view function for admin
...
# ------------------------------------------------------------------
# create a compplaintReply function for admin
def complaintReply(request,pk):
    varible= get_object_or_404(ComplaintModel, pk=pk)
    if request.method == 'POST':
        form = ComplaintReplyForm(request.POST,instance=varible)
        if form.is_valid():
            form.save()
            return redirect('complaintsTable')
    else:
        form = ComplaintReplyForm()
    return render(request, 'complaintReply.html', {'form': form})
# ------------------------------------------------------------------
# create a function to view complaintReply view
def usercomplaintReplyView(request): 
